...
function isArchivedJob(job,now=new Date()){const created=new Date(job.createdAt||0);if(Number.isNaN(created.getTime()))return false;if(localDayKey(created)!==localDayKey(now))return true;const cutoff=new Date(now);cutoff.setHours(20,0,0,0);return now>=cutoff}
